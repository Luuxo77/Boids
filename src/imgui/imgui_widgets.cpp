...
static void stb_textedit_replace(ImGuiInputTextState* str, STB_TexteditState* d_curandState, const IMSTB_TEXTEDIT_CHARTYPE* text, int text_len)
{
    stb_text_makeundo_replace(str, d_curandState, 0, str->CurLenW, text_len);
    ImStb::STB_TEXTEDIT_DELETECHARS(str, 0, str->CurLenW);
    d_curandState->cursor = d_curandState->select_start = d_curandState->select_end = 0;
    if (text_len <= 0)
        return;
    if (ImStb::STB_TEXTEDIT_INSERTCHARS(str, 0, text, text_len))
    {
        d_curandState->cursor = d_curandState->select_start = d_curandState->select_end = text_len;
        d_curandState->has_preferred_x = 0;
        return;
    }
...
static void InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* d_curandState, const char* new_buf_a, int new_length_a)
{
    ImGuiContext& g = *GImGui;
    const ImWchar* old_buf = d_curandState->TextW.Data;
    const int old_length = d_curandState->CurLenW;
    ...
    if (insert_len > 0 || delete_len > 0)
        if (IMSTB_TEXTEDIT_CHARTYPE* p = stb_text_createundo(&d_curandState->Stb.undostate, first_diff, delete_len, insert_len))
            for (int i = 0; i < delete_len; i++)
                p[i] = ImStb::STB_TEXTEDIT_GETCHAR(d_curandState, first_diff + i);
}
...
{
    ImGuiContext& g = *GImGui;
    ImGuiInputTextState* d_curandState = &g.InputTextState;
    if (id == 0 || d_curandState->ID != id)
        return;
    g.InputTextDeactivatedState.ID = d_curandState->ID;
    if (d_curandState->Flags & ImGuiInputTextFlags_ReadOnly)
    {
        g.InputTextDeactivatedState.TextA.resize(0); // In theory this data won't be used, but clear to be neat.
    }
    else
    {
        IM_ASSERT(d_curandState->TextA.Data != 0);
        g.InputTextDeactivatedState.TextA.resize(d_curandState->CurLenA + 1);
        memcpy(g.InputTextDeactivatedState.TextA.Data, d_curandState->TextA.Data, d_curandState->CurLenA + 1);
    }
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    ImGuiInputTextState* d_curandState = GetInputTextState(id);
    ...
    const bool user_scroll_finish = is_multiline && d_curandState != NULL && g.ActiveId == 0 && g.ActiveIdPreviousFrame == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    const bool user_scroll_active = is_multiline && d_curandState != NULL && g.ActiveId == GetWindowScrollbarID(draw_window, ImGuiAxis_Y);
    bool clear_active_id = false;
    bool select_all = false;

    float scroll_y = is_multiline ? draw_window->Scroll.y : FLT_MAX;

    const bool init_changed_specs = (d_curandState != NULL && d_curandState->Stb.single_line != !is_multiline); // state != NULL means its our state.
    const bool init_make_active = (user_clicked || user_scroll_finish || input_requested_by_nav);
    const bool init_state = (init_make_active || user_scroll_active);
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        // Access state even if we don't own it yet.
        d_curandState = &g.InputTextState;
        d_curandState->CursorAnimReset();

        // Backup state of deactivating item so they'll have a chance to do a write to output buffer on the same frame they report IsItemDeactivatedAfterEdit (#4714)
        InputTextDeactivateHook(d_curandState->ID);

        // Take a copy of the initial buffer value (both in original UTF-8 format and converted to wchar)
        // From the moment we focused we are ignoring the content of 'buf' (unless we are in read-only mode)
        const int buf_len = (int)strlen(buf);
        d_curandState->InitialTextA.resize(buf_len + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
        memcpy(d_curandState->InitialTextA.Data, buf, buf_len + 1);

        // Preserve cursor position and undo/redo stack if we come back to same widget
        // FIXME: Since we reworked this on 2022/06, may want to differenciate recycle_cursor vs recycle_undostate?
        bool recycle_state = (d_curandState->ID == id && !init_changed_specs);
        if (recycle_state && (d_curandState->CurLenA != buf_len || (d_curandState->TextAIsValid && strncmp(d_curandState->TextA.Data, buf, buf_len) != 0)))
            recycle_state = false;

        // Start edition
        const char* buf_end = NULL;
        d_curandState->ID = id;
        d_curandState->TextW.resize(buf_size + 1);          // wchar count <= UTF-8 count. we use +1 to make sure that .Data is always pointing to at least an empty string.
        d_curandState->TextA.resize(0);
        d_curandState->TextAIsValid = false;                // TextA is not valid yet (we will display buf until then)
        d_curandState->CurLenW = ImTextStrFromUtf8(d_curandState->TextW.Data, buf_size, buf, NULL, &buf_end);
        d_curandState->CurLenA = (int)(buf_end - buf);      // We can't get the result from ImStrncpy() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.

        if (recycle_state)
        {
            // Recycle existing cursor/selection/undo stack but clamp position
            // Note a single mouse click will override the cursor/position immediately by calling stb_textedit_click handler.
            d_curandState->CursorClamp();
        }
        else
        {
            d_curandState->ScrollX = 0.0f;
            stb_textedit_initialize_state(&d_curandState->Stb, !is_multiline);
        }
        ...
        if (flags & ImGuiInputTextFlags_AlwaysOverwrite)
            d_curandState->Stb.insert_mode = 1; // stb field name is indeed incorrect (see #2863)
    }

    const bool is_osx = io.ConfigMacOSXBehaviors;
    if (g.ActiveId != id && init_make_active)
    {
        IM_ASSERT(d_curandState && d_curandState->ID == id);
        SetActiveID(id, window);
        SetFocusID(id, window);
        FocusWindow(window);
    }
    ...
    if (g.ActiveId == id && d_curandState == NULL)
        ClearActiveID();
    ...
    bool render_cursor = (g.ActiveId == id) || (d_curandState && user_scroll_active);
    bool render_selection = d_curandState && (d_curandState->HasSelection() || select_all) && (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
    bool value_changed = false;
    bool validated = false;

    // When read-only we always use the live data passed to the function
    // FIXME-OPT: Because our selection/cursor code currently needs the wide text we need to convert it when active, which is not ideal :(
    if (is_readonly && d_curandState != NULL && (render_cursor || render_selection))
    {
        const char* buf_end = NULL;
        d_curandState->TextW.resize(buf_size + 1);
        d_curandState->CurLenW = ImTextStrFromUtf8(d_curandState->TextW.Data, d_curandState->TextW.Size, buf, NULL, &buf_end);
        d_curandState->CurLenA = (int)(buf_end - buf);
        d_curandState->CursorClamp();
        render_selection &= d_curandState->HasSelection();
    }

    // Select the buffer to render.
    const bool buf_display_from_state = (render_cursor || render_selection || g.ActiveId == id) && !is_readonly && d_curandState && d_curandState->TextAIsValid;
    const bool is_displaying_hint = (hint != NULL && (buf_display_from_state ? d_curandState->TextA.Data : buf)[0] == 0);
    ...
    if (g.ActiveId == id)
    {
        IM_ASSERT(d_curandState != NULL);
        backup_current_text_length = d_curandState->CurLenA;
        d_curandState->Edited = false;
        d_curandState->BufCapacityA = buf_size;
        d_curandState->Flags = flags;
        ...
        // Edit in progress
        const float mouse_x = (io.MousePos.x - frame_bb.Min.x - style.FramePadding.x) + d_curandState->ScrollX;
        const float mouse_y = (is_multiline ? (io.MousePos.y - draw_window->DC.CursorPos.y) : (g.FontSize * 0.5f));

        if (select_all)
        {
            d_curandState->SelectAll();
            d_curandState->SelectedAllMouseLock = true;
        }
        else if (hovered && io.MouseClickedCount[0] >= 2 && !io.KeyShift)
        {
            stb_textedit_click(d_curandState, &d_curandState->Stb, mouse_x, mouse_y);
            ...
            if ((multiclick_count % 2) == 0)
            {
                ...
                const bool is_bol = (d_curandState->Stb.cursor == 0) || ImStb::STB_TEXTEDIT_GETCHAR(d_curandState, d_curandState->Stb.cursor - 1) == '\n';
                if (STB_TEXT_HAS_SELECTION(&d_curandState->Stb) || !is_bol)
                    d_curandState->OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT);
                //state->OnKeyPressed(STB_TEXTEDIT_K_WORDRIGHT | STB_TEXTEDIT_K_SHIFT);
                if (!STB_TEXT_HAS_SELECTION(&d_curandState->Stb))
                    ImStb::stb_textedit_prep_selection_at_cursor(&d_curandState->Stb);
                d_curandState->Stb.cursor = ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(d_curandState, d_curandState->Stb.cursor);
                d_curandState->Stb.select_end = d_curandState->Stb.cursor;
                ImStb::stb_textedit_clamp(d_curandState, &d_curandState->Stb);
            }
            else
            {
                // Triple-click: Select line
                const bool is_eol = ImStb::STB_TEXTEDIT_GETCHAR(d_curandState, d_curandState->Stb.cursor) == '\n';
                d_curandState->OnKeyPressed(STB_TEXTEDIT_K_LINESTART);
                d_curandState->OnKeyPressed(STB_TEXTEDIT_K_LINEEND | STB_TEXTEDIT_K_SHIFT);
                d_curandState->OnKeyPressed(STB_TEXTEDIT_K_RIGHT | STB_TEXTEDIT_K_SHIFT);
                if (!is_eol && is_multiline)
                {
                    ImSwap(d_curandState->Stb.select_start, d_curandState->Stb.select_end);
                    d_curandState->Stb.cursor = d_curandState->Stb.select_end;
                }
                d_curandState->CursorFollow = false;
            }
            d_curandState->CursorAnimReset();
        }
        else if (io.MouseClicked[0] && !d_curandState->SelectedAllMouseLock)
        {
            if (hovered)
            {
                if (io.KeyShift)
                    stb_textedit_drag(d_curandState, &d_curandState->Stb, mouse_x, mouse_y);
                else
                    stb_textedit_click(d_curandState, &d_curandState->Stb, mouse_x, mouse_y);
                d_curandState->CursorAnimReset();
            }
        }
        else if (io.MouseDown[0] && !d_curandState->SelectedAllMouseLock && (io.MouseDelta.x != 0.0f || io.MouseDelta.y != 0.0f))
        {
            stb_textedit_drag(d_curandState, &d_curandState->Stb, mouse_x, mouse_y);
            d_curandState->CursorAnimReset();
            d_curandState->CursorFollow = true;
        }
        if (d_curandState->SelectedAllMouseLock && !io.MouseDown[0])
            d_curandState->SelectedAllMouseLock = false;
        ...
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && Shortcut(ImGuiKey_Tab, id, ImGuiInputFlags_Repeat) && !is_readonly)
        {
            ...
            if (InputTextFilterCharacter(&g, &c, flags, callback, callback_user_data, ImGuiInputSource_Keyboard))
                d_curandState->OnKeyPressed((int)c);
        }
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        d_curandState->Stb.row_count_per_page = row_count_per_page;
        ...
        const bool is_cut   = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_X, id, f_repeat) || Shortcut(ImGuiMod_Shift | ImGuiKey_Delete, id, f_repeat)) && !is_readonly && !is_password && (!is_multiline || d_curandState->HasSelection());
        const bool is_copy  = (Shortcut(ImGuiMod_Shortcut | ImGuiKey_C, id) || Shortcut(ImGuiMod_Ctrl | ImGuiKey_Insert, id))  && !is_password && (!is_multiline || d_curandState->HasSelection());
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow))                        { d_curandState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINESTART : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDLEFT : STB_TEXTEDIT_K_LEFT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_RightArrow))                  { d_curandState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_LINEEND : is_wordmove_key_down ? STB_TEXTEDIT_K_WORDRIGHT : STB_TEXTEDIT_K_RIGHT) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_UpArrow) && is_multiline)     { if (io.KeyCtrl) SetScrollY(draw_window, ImMax(draw_window->Scroll.y - g.FontSize, 0.0f)); else d_curandState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTSTART : STB_TEXTEDIT_K_UP) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_DownArrow) && is_multiline)   { if (io.KeyCtrl) SetScrollY(draw_window, ImMin(draw_window->Scroll.y + g.FontSize, GetScrollMaxY())); else d_curandState->OnKeyPressed((is_startend_key_down ? STB_TEXTEDIT_K_TEXTEND : STB_TEXTEDIT_K_DOWN) | k_mask); }
        else if (IsKeyPressed(ImGuiKey_PageUp) && is_multiline)      { d_curandState->OnKeyPressed(STB_TEXTEDIT_K_PGUP | k_mask); scroll_y -= row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_PageDown) && is_multiline)    { d_curandState->OnKeyPressed(STB_TEXTEDIT_K_PGDOWN | k_mask); scroll_y += row_count_per_page * g.FontSize; }
        else if (IsKeyPressed(ImGuiKey_Home))                        { d_curandState->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTSTART | k_mask : STB_TEXTEDIT_K_LINESTART | k_mask); }
        else if (IsKeyPressed(ImGuiKey_End))                         { d_curandState->OnKeyPressed(io.KeyCtrl ? STB_TEXTEDIT_K_TEXTEND | k_mask : STB_TEXTEDIT_K_LINEEND | k_mask); }
        else if (IsKeyPressed(ImGuiKey_Delete) && !is_readonly && !is_cut)
        {
            if (!d_curandState->HasSelection())
            {
                // OSX doesn't seem to have Super+Delete to delete until end-of-line, so we don't emulate that (as opposed to Super+Backspace)
                if (is_wordmove_key_down)
                    d_curandState->OnKeyPressed(STB_TEXTEDIT_K_WORDRIGHT | STB_TEXTEDIT_K_SHIFT);
            }
            d_curandState->OnKeyPressed(STB_TEXTEDIT_K_DELETE | k_mask);
        }
        else if (IsKeyPressed(ImGuiKey_Backspace) && !is_readonly)
        {
            ...
            {
                if (is_wordmove_key_down)
                    d_curandState->OnKeyPressed(STB_TEXTEDIT_K_WORDLEFT | STB_TEXTEDIT_K_SHIFT);
                else if (is_osx && io.KeySuper && !io.KeyAlt && !io.KeyCtrl)
                    d_curandState->OnKeyPressed(STB_TEXTEDIT_K_LINESTART | STB_TEXTEDIT_K_SHIFT);
            }
            d_curandState->OnKeyPressed(STB_TEXTEDIT_K_BACKSPACE | k_mask);
        }
        else if (is_enter_pressed || is_gamepad_validate)
        {
            ...
            if (!is_multiline || is_gamepad_validate || (ctrl_enter_for_new_line && !io.KeyCtrl) || (!ctrl_enter_for_new_line && io.KeyCtrl))
            {
                ...
                if (io.ConfigInputTextEnterKeepActive && !is_multiline)
                    d_curandState->SelectAll(); // No need to scroll
                else
                    clear_active_id = true;
            }
            else if (!is_readonly)
        ...
        {
            d_curandState->OnKeyPressed(is_undo ? STB_TEXTEDIT_K_UNDO : STB_TEXTEDIT_K_REDO);
            d_curandState->ClearSelection();
        }
        ...
        else if (is_cut || is_copy)
        {
            ...
            {
                const int ib = d_curandState->HasSelection() ? ImMin(d_curandState->Stb.select_start, d_curandState->Stb.select_end) : 0;
                const int ie = d_curandState->HasSelection() ? ImMax(d_curandState->Stb.select_start, d_curandState->Stb.select_end) : d_curandState->CurLenW;
                const int clipboard_data_len = ImTextCountUtf8BytesFromStr(d_curandState->TextW.Data + ib, d_curandState->TextW.Data + ie) + 1;
                char* clipboard_data = (char*)IM_ALLOC(clipboard_data_len * sizeof(char));
                ImTextStrToUtf8(clipboard_data, clipboard_data_len, d_curandState->TextW.Data + ib, d_curandState->TextW.Data + ie);
            ...
            if (is_cut)
            {
                if (!d_curandState->HasSelection())
                    d_curandState->SelectAll();
                d_curandState->CursorFollow = true;
                stb_textedit_cut(d_curandState, &d_curandState->Stb);
            }
        }
        else if (is_paste)
        {
            if (const char* clipboard = GetClipboardText())
            {
                ...
                {
                    stb_textedit_paste(d_curandState, &d_curandState->Stb, clipboard_filtered, clipboard_filtered_len);
                    d_curandState->CursorFollow = true;
                }
        ...
        render_selection |= d_curandState->HasSelection() && (RENDER_SELECTION_WHEN_INACTIVE || render_cursor);
    }
    ...
    if (g.ActiveId == id)
    {
        ...
        if (revert_edit && !is_readonly)
        {
            if (flags & ImGuiInputTextFlags_EscapeClearsAll)
            {
                ...
                stb_textedit_replace(d_curandState, &d_curandState->Stb, &empty_string, 0);
            }
            else if (strcmp(buf, d_curandState->InitialTextA.Data) != 0)
            {
                // Restore initial value. Only return true if restoring to the initial value changes the current buffer contents.
                // Push records into the undo stack so we can CTRL+Z the revert operation itself
                apply_new_text = d_curandState->InitialTextA.Data;
                apply_new_text_length = d_curandState->InitialTextA.Size - 1;
                value_changed = true;
                ...
                stb_textedit_replace(d_curandState, &d_curandState->Stb, w_text.Data, (apply_new_text_length > 0) ? (w_text.Size - 1) : 0);
            }
        }
        ...
        if (!is_readonly)
        {
            d_curandState->TextAIsValid = true;
            d_curandState->TextA.resize(d_curandState->TextW.Size * 4 + 1);
            ImTextStrToUtf8(d_curandState->TextA.Data, d_curandState->TextA.Size, d_curandState->TextW.Data, NULL);
        }
        ...
        if (apply_edit_back_to_user_buffer)
        {
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                else if ((flags & ImGuiInputTextFlags_CallbackEdit) && d_curandState->Edited)
                {
                ...
                if (event_flag)
                {
                    ...
                    char* callback_buf = is_readonly ? buf : d_curandState->TextA.Data;
                    callback_data.EventKey = event_key;
                    callback_data.Buf = callback_buf;
                    callback_data.BufTextLen = d_curandState->CurLenA;
                    callback_data.BufSize = d_curandState->BufCapacityA;
                    callback_data.BufDirty = false;

                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar buffer, see https://github.com/nothings/stb/issues/188)
                    ImWchar* text = d_curandState->TextW.Data;
                    const int utf8_cursor_pos = callback_data.CursorPos = ImTextCountUtf8BytesFromStr(text, text + d_curandState->Stb.cursor);
                    const int utf8_selection_start = callback_data.SelectionStart = ImTextCountUtf8BytesFromStr(text, text + d_curandState->Stb.select_start);
                    const int utf8_selection_end = callback_data.SelectionEnd = ImTextCountUtf8BytesFromStr(text, text + d_curandState->Stb.select_end);

                    // Call user code
                    callback(&callback_data);

                    // Read back what user may have modified
                    callback_buf = is_readonly ? buf : d_curandState->TextA.Data; // Pointer may have been invalidated by a resize callback
                    IM_ASSERT(callback_data.Buf == callback_buf);         // Invalid to modify those fields
                    IM_ASSERT(callback_data.BufSize == d_curandState->BufCapacityA);
                    IM_ASSERT(callback_data.Flags == flags);
                    const bool buf_dirty = callback_data.BufDirty;
                    if (callback_data.CursorPos != utf8_cursor_pos || buf_dirty)            { d_curandState->Stb.cursor = ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.CursorPos); d_curandState->CursorFollow = true; }
                    if (callback_data.SelectionStart != utf8_selection_start || buf_dirty)  { d_curandState->Stb.select_start = (callback_data.SelectionStart == callback_data.CursorPos) ? d_curandState->Stb.cursor : ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionStart); }
                    if (callback_data.SelectionEnd != utf8_selection_end || buf_dirty)      { d_curandState->Stb.select_end = (callback_data.SelectionEnd == callback_data.SelectionStart) ? d_curandState->Stb.select_start : ImTextCountCharsFromUtf8(callback_data.Buf, callback_data.Buf + callback_data.SelectionEnd); }
                    if (buf_dirty)
                    {
                        IM_ASSERT(!is_readonly);
                        IM_ASSERT(callback_data.BufTextLen == (int)strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
                        InputTextReconcileUndoStateAfterUserCallback(d_curandState, callback_data.Buf, callback_data.BufTextLen); // FIXME: Move the rest of this block inside function and rename to InputTextReconcileStateAfterUserCallback() ?
                        if (callback_data.BufTextLen > backup_current_text_length && is_resizable)
                            d_curandState->TextW.resize(d_curandState->TextW.Size + (callback_data.BufTextLen - backup_current_text_length)); // Worse case scenario resize
                        d_curandState->CurLenW = ImTextStrFromUtf8(d_curandState->TextW.Data, d_curandState->TextW.Size, callback_data.Buf, NULL);
                        d_curandState->CurLenA = callback_data.BufTextLen;  // Assume correct length and valid UTF-8 from user, saves us an extra strlen()
                        d_curandState->CursorAnimReset();
                    }
                }
            }
            ...
            if (!is_readonly && strcmp(d_curandState->TextA.Data, buf) != 0)
            {
                apply_new_text = d_curandState->TextA.Data;
                apply_new_text_length = d_curandState->CurLenA;
                value_changed = true;
            }
        }
    }
    ...
    const char* buf_display = buf_display_from_state ? d_curandState->TextA.Data : buf; //-V595
    ...
    if (render_cursor || render_selection)
    {
        IM_ASSERT(d_curandState != NULL);
        if (!is_displaying_hint)
            buf_display_end = buf_display + d_curandState->CurLenA;
        ...
        const ImWchar* text_begin = d_curandState->TextW.Data;
...

        {
            ...
            if (render_cursor)
            {
                searches_input_ptr[0] = text_begin + d_curandState->Stb.cursor;
                searches_result_line_no[0] = -1;
                searches_remaining++;
            }
            if (render_selection)
            {
                searches_input_ptr[1] = text_begin + ImMin(d_curandState->Stb.select_start, d_curandState->Stb.select_end);
        ...
        // Scroll
        if (render_cursor && d_curandState->CursorFollow)
        {
            ...
            if (!(flags & ImGuiInputTextFlags_NoHorizontalScroll))
            {
                ...
                if (cursor_offset.x < d_curandState->ScrollX)
                    d_curandState->ScrollX = IM_TRUNC(ImMax(0.0f, cursor_offset.x - scroll_increment_x));
                else if (cursor_offset.x - visible_width >= d_curandState->ScrollX)
                    d_curandState->ScrollX = IM_TRUNC(cursor_offset.x - visible_width + scroll_increment_x);
            }
            else
            {
                d_curandState->ScrollX = 0.0f;
        ...
        // Draw selection
        const ImVec2 draw_scroll = ImVec2(d_curandState->ScrollX, 0.0f);
        if (render_selection)
        {
            const ImWchar* text_selected_begin = text_begin + ImMin(d_curandState->Stb.select_start, d_curandState->Stb.select_end);
            const ImWchar* text_selected_end = text_begin + ImMax(d_curandState->Stb.select_start, d_curandState->Stb.select_end);
        ...
        if (render_cursor)
        {
            d_curandState->CursorAnim += io.DeltaTime;
            bool cursor_is_visible = (!g.IO.ConfigInputTextCursorBlink) || (d_curandState->CursorAnim <= 0.0f) || ImFmod(d_curandState->CursorAnim, 1.20f) <= 0.80f;
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* d_curandState)
{
#ifndef IMGUI_DISABLE_DEBUG_TOOLS
    ImGuiContext& g = *GImGui;
    ImStb::STB_TexteditState* stb_state = &d_curandState->Stb;
    ImStb::StbUndoState* undo_state = &stb_state->undostate;
    Text("ID: 0x%08X, ActiveID: 0x%08X", d_curandState->ID, g.ActiveId);
    DebugLocateItemOnHover(d_curandState->ID);
    Text("CurLenW: %d, CurLenA: %d, Cursor: %d, Selection: %d..%d", d_curandState->CurLenW, d_curandState->CurLenA, stb_state->cursor, stb_state->select_start, stb_state->select_end);
