...
void ImGui::TableAngledHeadersRowEx(float viewAngle, float max_label_width)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    const bool flip_label = (viewAngle < 0.0f);
    viewAngle -= IM_PI * 0.5f;
    const float cos_a = ImCos(viewAngle);
    const float sin_a = ImSin(viewAngle);
    const float label_cos_a = flip_label ? ImCos(viewAngle + IM_PI) : cos_a;
    const float label_sin_a = flip_label ? ImSin(viewAngle + IM_PI) : sin_a;
